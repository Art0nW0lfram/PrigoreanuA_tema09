...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            GL.PushMatrix();
            GL.Translate(0, 20, 0); // O ridicăm puțin mai sus
            DeseneazaTriunghiTexturat();
            GL.PopMatrix();
        ...
        private void DeseneazaTriunghiTexturat()
        {
            // 1. Activăm texturarea
            GL.Enable(EnableCap.Texture2D);

            // 2. Alegem textura (folosim textura 0 - cea cu cărămizi/brick din exemplul anterior)
            // Asigură-te că vectorul 'textures' este inițializat corect în LoadTextures()
            GL.BindTexture(TextureTarget.Texture2D, textures[0]);

            // 3. Setăm culoarea albă pentru a nu altera culorile originale ale imaginii
            GL.Color3(Color.White);

            GL.Begin(PrimitiveType.Triangles);

            // --- VERTEX 1: Stânga-Jos ---
            // Coordonata Textură (0, 0) = Colțul stânga-jos al imaginii
            GL.TexCoord2(0.0f, 0.0f);
            GL.Vertex3(-20.0f, 0.0f, 0.0f); // Poziție 3D în scenă

            // --- VERTEX 2: Dreapta-Jos ---
            // Coordonata Textură (1, 0) = Colțul dreapta-jos al imaginii
            GL.TexCoord2(1.0f, 0.0f);
            GL.Vertex3(20.0f, 0.0f, 0.0f); // Poziție 3D în scenă

            // --- VERTEX 3: Vârf (Sus-Centru) ---
            // AICI ESTE CHEIA: Alegem mijlocul laturii de sus a imaginii (0.5, 1.0)
            // Astfel, imaginea pătrată este "strânsă" (pinched) spre vârf.
            GL.TexCoord2(0.5f, 1.0f);
            GL.Vertex3(0.0f, 40.0f, 0.0f); // Poziție 3D în scenă

            GL.End();

            // Opțional: Dezactivăm texturarea dacă urmează să desenăm obiecte colorate simplu
